...
export async function captureUrls(
  ...
  progress?.line("Launching Chromium...");
  ...
  progress?.found(items.length, "URLs to capture");
  try {
    for (let index = 0; index < items.length; index++) {
      ...
      let context,
        ...
        observed = [];
      try {
        progress?.start(index + 1, items.length, "Capturing", url);
        context = await browser.newContext({
          recordHar: { path: har, mode: "full", content: "embed" },
        });
        try {
          await context.addInitScript(dataLayerInitScript(), DATA_LAYER_LIMITS);
        ...
        const forms = await detectForms(page, finalUrl);
        ...
        result.completed++;
        outcome = "completed";
        progress?.success(index + 1, items.length, "Captured", finalUrl);
      } catch (e) {
        result.errors.push(
          error(run.mode, url, null, null, null, "capture", e),
        );
        progress?.failure(index + 1, items.length, url, e.message);
      } finally {
        if (context) {
          try {
            const page = context.pages()[0];
            const records = page
              ? await page.evaluate(() => window.__siteAuditDataLayer || [])
              : [];
            const companion = companionPath(har);
            await fs.writeFile(
              companion,
              JSON.stringify({
                capture_id: path.basename(har, ".har"),
                requested_page_url: url,
                final_page_url: finalUrl,
                records,
                instrumentation_error: null,
                limits: DATA_LAYER_LIMITS,
              }, null, 2),
            );
            result.dataLayerFiles.push(companion);
          } catch (e) {
            result.errors.push(error(run.mode, url, har, null, null, "datalayer_instrumentation", e));
          }
          await context.close();
          try {
      ...
      result.manifest.push({
        ...
        datalayer_file: path.basename(companionPath(har)),
      });
      result.harFiles.push(har);
    }
  } finally {
    progress?.line("Closing Chromium...");
    await browser.close();
  }
  return result;
}

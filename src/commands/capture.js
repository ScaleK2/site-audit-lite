...
export async function captureList({ urls, outputRoot, waitMs }) {
...
export async function captureSite({
  ...
  while (queue.length && seen.size < maxUrls) {
    ...
    const c = await import("../capture/browser.js").then((m) =>
      m.captureUrls(run, [u], { waitMs, discover: true }),
    );
    merge(all, c);
  ...
  return finalise(run, all, [], { discovered_count: seen.size });
}
export async function captureConfig({
  ...
  return finish(
    run,
    valid,
    { waitMs, interactions: cfg.interactions, confirmSubmissions },
    errs,
  );
}
async function finish(run, urls, opts, errs) {
  const { captureUrls } = await import("../capture/browser.js");
  return finalise(run, await captureUrls(run, urls, opts), errs);
}
async function finalise(run, c, initial = [], extra = {}) {
  let requests = [];
...
function merge(a, b) {
  for (const k of ["harFiles", "manifest", "forms", "templates", "errors"])
    a[k].push(...b[k]);
